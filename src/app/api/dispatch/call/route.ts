...
export async function POST(request: Request) {
  ...
  if (!parsed.success) {
    console.error("Dispatch validation failed: call", {
      payload: json,
  ...
  const result = await sendCallDispatch(parsed.data);
  ...
  return NextResponse.json({
    ...
    provider: "mock",
    result,
    message: `Mock Call dispatch accepted for ${parsed.data.worker_ids.length} worker(s).`,
  });
}
