-- Local PostgreSQL schema for the workforce dispatch test environment.
-- This mirrors the existing domain model without changing Supabase assets.

create extension if not exists pgcrypto;

-- Shared trigger to keep updated_at fresh across mutable tables.
...
create table if not exists jobs (
  ...
  provider_id uuid not null references job_providers(provider_id) on delete restrict,
  ...
  broadcast_status text not null default 'broadcast ready' check (broadcast_status in ('broadcast ready', 'awaiting response', 'completed', 'failed', 'cancelled')),
...
create table if not exists staff_subs (
...
create table if not exists bookings (
  ...
  job_id uuid not null references jobs(job_id) on delete cascade,
  worker_id uuid not null references staff_subs(worker_id) on delete restrict,
...
create table if not exists compliance_legal (
  id uuid primary key default gen_random_uuid(),
  provider_id uuid references job_providers(provider_id) on delete cascade,
  worker_id uuid references staff_subs(worker_id) on delete cascade,
  contract_signed boolean not null default false,
  non_circumvention boolean not null default false,
...
-- Indexes for matching, dispatch, and booking lookups.
create index if not exists idx_jobs_provider_id on jobs(provider_id);
create index if not exists idx_jobs_status_dates on jobs(job_status, start_date, postcode);
create index if not exists idx_jobs_skill_tags on jobs using gin (skill_tags);
create index if not exists idx_staff_subs_status_availability on staff_subs(status, available_today, worker_type);
create index if not exists idx_staff_subs_skill_tags on staff_subs using gin (skill_tags);
create index if not exists idx_staff_subs_postcode on staff_subs(postcode);
create index if not exists idx_response_log_job_response_time on response_log(job_id, response_time);
create index if not exists idx_response_log_worker on response_log(worker_id);
...
create index if not exists idx_bookings_job_status on bookings(job_id, booking_status);
create index if not exists idx_compliance_provider on compliance_legal(provider_id);
create index if not exists idx_compliance_worker on compliance_legal(worker_id);

-- Response ranking is maintained so first responders can be selected reliably.
create or replace function recalculate_response_ranks(target_job_id uuid)
returns void
language sql
as $$
  with ranked as (
  ...
  update response_log rl
  set response_rank = ranked.computed_rank
  from ranked
  where rl.response_id = ranked.response_id;
$$;

create or replace function handle_response_ranking()
returns trigger
language plpgsql
as $$
begin
  if tg_op = 'DELETE' then
    perform recalculate_response_ranks(old.job_id);
    return old;
  end if;
  ...
  perform recalculate_response_ranks(new.job_id);
  return coalesce(new, old);
end;
$$;

-- Booking changes update the aggregate worker count on each job.
...
-- Provider metrics are rolled up from jobs for quick visibility.
...
begin
  ...
  update job_providers p
  set
    total_jobs = (
      select count(*) from jobs j where j.provider_id = target_provider_id
    ),
    ...
    first_job_date = (
      select min(j.start_date) from jobs j where j.provider_id = target_provider_id
    ),
    last_job_date = (
      select max(j.start_date) from jobs j where j.provider_id = target_provider_id
    )
  ...
  if tg_op = 'UPDATE' and new.provider_id is distinct from old.provider_id then
    ...
    set
      total_jobs = (
        select count(*) from jobs j where j.provider_id = old.provider_id
      ),
      total_revenue = coalesce((
        select sum(coalesce(j.charge_rate, 0) * coalesce(j.workers_confirmed, 0))
        from jobs j
      ...
      first_job_date = (
        select min(j.start_date) from jobs j where j.provider_id = old.provider_id
      ),
      last_job_date = (
        select max(j.start_date) from jobs j where j.provider_id = old.provider_id
      )
...
drop trigger if exists trg_job_providers_set_updated_at on job_providers;
...
drop trigger if exists trg_staff_subs_set_updated_at on staff_subs;
create trigger trg_staff_subs_set_updated_at
before update on staff_subs
for each row
execute function set_updated_at();

drop trigger if exists trg_compliance_legal_set_updated_at on compliance_legal;
create trigger trg_compliance_legal_set_updated_at
before update on compliance_legal
for each row
execute function set_updated_at();

drop trigger if exists trg_response_log_rank on response_log;
create trigger trg_response_log_rank
after insert or update of response_time, job_id, created_at, response_type, selected on response_log
for each row
execute function handle_response_ranking();

drop trigger if exists trg_response_log_rank_delete on response_log;
create trigger trg_response_log_rank_delete
after delete on response_log
for each row
execute function handle_response_ranking();

drop trigger if exists trg_bookings_sync_confirmed on bookings;
create trigger trg_bookings_sync_confirmed
after insert or update of booking_status, job_id, worker_id, confirmed_time, check_in, check_out, timesheet_received on bookings
for each row
execute function sync_workers_confirmed();

drop trigger if exists trg_bookings_sync_confirmed_delete on bookings;
create trigger trg_bookings_sync_confirmed_delete
after delete on bookings
for each row
execute function sync_workers_confirmed();

drop trigger if exists trg_jobs_update_provider_metrics on jobs;
